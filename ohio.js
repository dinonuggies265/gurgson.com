...
const upgrades = {
    ...
    "oven-mitts": {
        name: "Oven Mitts",
        description: "Oven mitts to help you take cookies out of the oven without burning your fingies",
        price: 1000,
        effects: {
            add: {
                clickPower: 1,
...
function achievementsCheckTick() {
    for (const achievementName in achievements) {
        ...
        if (achievement.requirements() && !(game.achievementsGained.includes(achievementName))) {
...
function updateText() {
   ...
   document.title = `GreyClicker - ${game.cookies.toFixed(2)} cookies`
    for (upgradeName in upgrades) {
        ...
        upgradeHeader.querySelector(".buildingsOwnedIndicator").innerHTML = `(${game.buildings[upgradeName]})` 
    }
}
...
for (const upgradeName in upgrades) {
    ...
    elementHeader.append(buildingsOwnedIndicator, document.createTextNode(` ${upgrade.name} - $${upgrade.price}`))
    element.id = upgradeName

    const elementDescription = document.createElement("p")
    elementDescription.textContent = upgrade.description + Object.keys(upgrade.effects.add).map(effectName => {
...
setInterval(() => {
    ...
    for (building in game.buildings) {
        ...
        Object.keys(upgrades[building].effects.add).forEach(effect => {
            game.stats[effect] += upgrades[building].effects.add[effect] * buildingsOwned
